...
// #ifdef HAND_LIGHT
// 	float3 uv2            : TEXCOORD2;  //rgb: hand light direction
...
#ifdef LIGHT_SHADOW
	output.uv0 = float3(input.uv0, 1.0) * float3(1.0/4096, 1.0/4096, 1.0);
...
// 	output.uv2 = g_HandLightPos.xyz - input.pos.xyz;
// #endif
...
// return float4(shadow,shadow,shadow,1);
	...
	// float gray = dot( tcolor.xyz, float3(0.3,0.59,0.11) );
	// tcolor.xyz = lerp( float3(gray,gray,gray), tcolor.xyz, g_vSaturation);
	// tcolor.xyz = tcolor.xyz * g_vColorScale;    
  	color.rgb += tcolor.rgb;
#endif

#if defined(FOG_DISTANCE)
	//color.rgb = lerp(input.color1.rgb, color.rgb, input.color1.a);
